defmodule LightningGraph.Lnd.FileConverter do
  require Logger
  ...
  defp extract_channels json_filename, output_filename do
    ...
    select = "[\"channel_id\", \"node1_pub\", \"node2_pub\", \"capacity\", \"base_fee\", \"fee_rate\", \"is_disabled\"]"
    filter1 = "(.edges[] | select(.last_update > 0) | [.channel_id, .node1_pub, .node2_pub, (.capacity|tonumber), (.node1_policy.fee_base_msat // '0'|tonumber), (.node1_policy.fee_rate_milli_msat // '0'|tonumber), (if .node1_policy.disabled then 1 else 0 end // 0)])"
    filter2 = "(.edges[] | select(.last_update > 0) | [.channel_id, .node2_pub, .node1_pub, (.capacity|tonumber), (.node2_policy.fee_base_msat // '0'|tonumber), (.node2_policy.fee_rate_milli_msat // '0'|tonumber), (if .node2_policy.disabled then 1 else 0 end // 0)])"
